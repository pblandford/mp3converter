...
import com.philblandford.mp3convertercore.engine.file.input.NoteOffEvent
import com.philblandford.mp3convertercore.engine.file.input.NoteOnEvent
import com.philblandford.mp3convertercore.engine.file.input.ProgramChangeEvent
...
class FluidSampler(private val soundFontPath: String) :
  ...
  override fun passEvent(midiEvent: MidiEvent) {
    ...
    when (midiEvent) {
      ...
      is NoteOnEvent -> noteOn(
          channel,
          midiEvent.midiVal,
          midiEvent.velocity
      )
      is NoteOffEvent -> noteOff(
          channel,
          midiEvent.midiVal
      )
    }
