...
void
fluid_channel_init(fluid_synth_t* synth, fluid_channel_t* chan)
{
  ...
  chan->interp_method = synth->interp;
  chan->tuning = NULL;
  chan->nrpn_select = 0;
  chan->nrpn_active = 0;
}
...
void
fluid_channel_reset(fluid_synth_t* synth, fluid_channel_t* chan)
{
  fluid_channel_init(synth, chan);
  fluid_channel_init_ctrl(chan);
}
